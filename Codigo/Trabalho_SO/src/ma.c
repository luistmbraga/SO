...
int main(int argc, char* argv[]){
	...
	while((size = readln(STDIN_FILENO, byte, 200)) > 0){
		...
		//mostra o nome e o preço do artigo
			...
			if(isDigit(lista[1])){
	    ...
	    else if(!strcmp(byte, "a")){
	    		mkfifo("ResultadoAgregar", 0600);
				int fdAgregar = open("Agregar", O_WRONLY);
				write(fdAgregar, "a\n", strlen("a\n"));
				close(fdAgregar);
 				int fd = open("ResultadoAgregar", O_RDONLY);
 				char c[100];
 				readln(fd, c, 100);
 				close(fd);
				...
				int fdAg = open(c, O_RDONLY);
				while(read(fdAg, v, sizeof(struct venda))){
					char charid[120];
					char charquant[30];
					char charvalor[30];
					sprintf(charid, "%d ", v->id);
					sprintf(charquant, "%d ", v->quantidade);
					sprintf(charvalor, "%d ", v->valor);
					strcat(charid, charquant);
					strcat(charid, charvalor);
					strcat(charid, "\n");
					write(STDOUT_FILENO, charid, strlen(charid));
				}
				unlink("ResultadoAgregar");
				close(fdAg);
